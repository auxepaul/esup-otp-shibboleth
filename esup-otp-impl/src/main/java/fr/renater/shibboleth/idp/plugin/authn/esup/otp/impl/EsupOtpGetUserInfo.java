...
/**
 * An action that get esup otp user informations from a username from a lookup
 * strategy, creates a {@link EsupOtpContext}, and attaches it to the
...
public class EsupOtpGetUserInfo extends AbstractAuthenticationAction {
    ...
    protected void doExecute(@Nonnull ProfileRequestContext profileRequestContext,
        ...
        try {
            ...
            log.debug("Transports possible for {} : {}", esupOtpContext.getUsername(),
                    esupOtpContext.getConfiguredTransports());
     ...
     *            list of methods configured in properties
     * @param userInfo
     *            receive from esup-otp-api
    ...
    private Set<String> getChoices(Set<String> supportedMethods, EsupOtpUserInfoResponse userInfo) {
        ...
        allUserMethodByType.entrySet().stream().filter(entry -> entry.getValue() != null).filter(entry -> {
            ...
            if (SUPPORTED_METHODS_WITHOUT_TRANSPORT.contains(entry.getKey())) {
                ...
                Set<String> choiceWithTransport = entry.getValue().getTransports().stream()
                        .map(transport -> entry.getKey() + "." + transport).collect(Collectors.toSet());
    ...
    private Map<String, String> getTransports(EsupOtpUserInfoResponse userInfo) {
        return userInfo.getUser().getTransports().getAll().entrySet().stream().filter(entry -> entry.getValue() != null)
